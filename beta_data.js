...
window.phraseDict = [
    ...
    ["vBeta 專屬更新說明 (對比正式版)", "vBeta 专属更新说明 (对比正式版)"],
    ["全新 App 級介面", "全新 App 级介面"],
    ["引進暗黑毛玻璃卡片佈局與合適排版比例，解決手機端破版與電腦版過寬問題。", "引进暗黑毛玻璃卡片布局与合适排版比例，解决手机端破版与电脑版过宽问题。"],
    ["智慧狀態指示燈", "智慧状态指示灯"],
    ["全選(綠)與清空(紅)按鈕將依據您的勾選狀態自動切換；過濾器採用高反差設計。", "全选(绿)与清空(红)按钮将依据您的勾选状态自动切换；过滤器采用高反差设计。"],
    ["拖曳式排兵布陣", "拖曳式排兵布阵"],
    ["在編隊卡片上，直接用滑鼠或手指「按住拖曳」即可無縫交換隊伍順序！", "在编队卡片上，直接用滑鼠或手指「按住拖曳」即可无缝交换队伍顺序！"],
    ["動態數量控制", "动态数量控制"],
    ["新增自訂「顯示編隊數量」選單，減少手機版不必要的滑動空間。", "新增自订「显示编队数量」选单，减少手机版不必要的滑动空间。"],
    ["本地記憶 10 套編隊", "本地记忆 10 套编队"],
    ["現在您可以將編好的隊伍命名並儲存，隨時在數據總管中無縫載入。", "现在您可以将编好的队伍命名并储存，随时在数据总管中无缝载入。"],
    ["雙模式推演引擎", "双模式推演引擎"],
    ["新增「🔥自動推演 (依DPS接力)」與「🗺️手動編隊 (根據殘血反算總分與置信度)」兩種模式。", "新增「🔥自动推演 (依DPS接力)」与「🗺️手动编队 (根据残血反算总分与置信度)」两种模式。"],
    ["進階截圖分享", "进阶截图分享"],
    ["新增分享全編制實戰得分、BOSS 殘血狀態，以及是否曾遭遇抗性之功能。", "新增分享全编制实战得分、BOSS 残血状态，以及是否曾遭遇抗性之功能。"],

    // 💡 介面短語與設定
    ["主輸出篩選", "主输出筛选"], ["推薦配隊快速填入", "推荐配队快速填入"], ["填入下一空位", "填入下一空位"],
    ["顯示隊伍數", "显示队伍数"], ["16 隊 (極限上限)", "16 队 (极限上限)"], ["12 隊", "12 队"], ["9 隊", "9 队"], ["6 隊", "6 队"], ["3 隊", "3 队"],
    ["🔥 自動推演 (依DPS接力)", "🔥 自动推演 (依DPS接力)"],
    ["🗺️ 手動編隊 (根據殘血反算總分與置信度)", "🗺️ 手动编队 (根据残血反算总分与置信度)"],
